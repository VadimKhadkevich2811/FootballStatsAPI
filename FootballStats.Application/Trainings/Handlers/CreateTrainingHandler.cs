...
public class CreateTrainingHandler : IRequestHandler<CreateTrainingCommand, Response<TrainingReadDto>>
{
    ...
    public async Task<Response<TrainingReadDto>> Handle(CreateTrainingCommand request, CancellationToken cancellationToken)
    {
        ...
        newTraining.TrainedPlayers = new List<PlayerReadDto>();
        foreach (var trainingPlayer in training.TrainingPlayers!)
        {
            var player = await _playersRepository.GetPlayerByIdAsync(trainingPlayer.PlayerId);
            var playerDto = _mapper.Map<PlayerReadDto>(player);
            newTraining.TrainedPlayers.Add(playerDto);
        }
